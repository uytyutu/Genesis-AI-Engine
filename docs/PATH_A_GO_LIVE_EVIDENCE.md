...
### Production Line — операционный слепок (канон 2026-07-18)

Virtus Core = небольшая **цифровая сервисная компания**, не «ИИ, который генерирует сайты».

| Отдел | Модуль | Роль | Авто vs человек |
|-------|--------|------|-----------------|
| Продажи | Country Desk | Поиск DE SMB → черновик → `/order` | Поиск/черновик авто; **send** — CEO + Impressum + `GENESIS_OUTREACH_ENABLED` |
| Интерфейс | Vector | Точка взаимодействия / RC1 | RC1 живёт **отдельно** от продажи лендингов |
| Производство | Factory | Оплаченный пакет → ZIP (контракт) | Авто: package + данные → артефакт |
| Финансы | Finance + Stripe | Checkout · сумма/валюта · settlement | Авто по контракту оплаты |
| R&D | Farm | Эксперименты / labeling | dry_run; **не** вмешивается в Path A |
| Руководитель | CEO | Go/No-Go, стратегия, риск | Человек |

**Frontline выручки:** только Path A (заказ → оплата → результат).  
**Параллельно можно:** Factory (исполнение) · Desk без массовой отправки · Finance · RC1 · Farm dry_run.  
**Не запускать сейчас:** TikTok · Trading · Marketing Engine · Reseller Hosting · PaymentCenter multi · themes/Brand Transfer.

**Принцип:** автоматизировать там, где контракт ясен; человеку — коммерческий и юридический риск.

**Язык готовности:** готов принять реквизиты Gewerbe и после Gate 1 начать **пилот**.  
Не говорить «полностью готов к коммерческой эксплуатации» до первого оплаченного клиента.

**Успех ближайшего месяца (не коммиты):** Gewerbe → Gate 1 PASS → первый € → сданный проект → `FIRST_CUSTOMER_POSTMORTEM.md`.
