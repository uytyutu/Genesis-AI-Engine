...
import { VirtusOpsStrip } from "../components/VirtusOpsStrip";
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-6 px-4 py-8 text-zinc-100">
      <div className="rounded-xl border border-amber-500/30 bg-amber-950/25 px-4 py-3 text-sm text-amber-100/90">
        <p className="font-semibold">Архив · старый CEO Dashboard</p>
        <p className="mt-1 text-xs text-amber-100/70">
          Ежедневная работа:{" "}
          <Link href="/executive" className="underline hover:text-white">
            Обзор
          </Link>
          . Этот экран сохранён для справки (Farm / labs / старые KPI).
        </p>
      </div>
      ...
      <VirtusOpsStrip
        ...
        revenueEur={v?.revenue_eur}
        websitesSold={v?.websites_sold}
        systemMark={data?.dashboard_health?.headline_ru}
      />
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        <Link href="/orders" className="text-emerald-200/90 hover:underline">
          → Orders
        </Link>
        <Link href="/factory" className="text-emerald-200/90 hover:underline">
          → Products
        </Link>
        ...
        <Link href="/farm" className="text-zinc-500 hover:underline">
          → Farm (archive)
        </Link>
      </div>
    </main>
