...
/** VCORE Exchangeability Engine — 10-step permissionless DEX assessment */
export async function GET(req: Request) {
  const offline = new URL(req.url).searchParams.get("offline") === "1";
  const root = path.resolve(process.cwd(), "..", "..");
  const args = ["-3.12", "-m", "virtus_core.vcore_exchangeability"];
  ...
  if (!result.ok) {
    return NextResponse.json(
      { ok: false, error: result.err || "exchangeability failed", stdout: result.out.slice(0, 800) },
      { status: 500 },
    );
  }
  try {
