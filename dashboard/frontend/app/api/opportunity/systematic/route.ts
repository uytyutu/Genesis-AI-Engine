...
/** Opportunity AI — Systematic Economic Discovery (honest NO_VALID_OPPORTUNITY allowed) */
export async function GET(req: Request) {
  const offline = new URL(req.url).searchParams.get("offline") === "1";
  const root = path.resolve(process.cwd(), "..", "..");
  const args = ["-3.12", "-m", "virtus_core.opportunity_ai"];
  ...
  if (!result.ok) {
    return NextResponse.json(
      { ok: false, error: result.err || "opportunity systematic failed", stdout: result.out.slice(0, 800) },
      { status: 500 },
    );
  }
  try {
