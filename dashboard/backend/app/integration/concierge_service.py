...
class ConciergeService:
    ...
    def ask(self, question: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if not q and not ctx.get("intent"):
            return self._reply(
                ...
                "или «хочу пользоваться Virtus Studio». Я подберу подходящий путь.",
            )
        ...
        if self._match(
            ...
            return self._reply(
                f"{BRAND_NAME} — интеллектуальная платформа с услугами под ключ и инструментами для самостоятельной работы.\n\n"
                f"**Услуги** — вы получаете готовый результат (сайт, магазин…), {BRAND_NAME} делает работу.\n"
                f"**{STUDIO_NAME}** — подписка на платформу для тех, кто хочет создавать проекты сам.\n\n"
                f"Я — {ASSISTANT_NAME}, интеллектуальный помощник платформы. Расскажите, что вам ближе — и я подскажу следующий шаг.",
            )
    ...
    def _studio_pitch(self) -> dict[str, Any]:
        return self._reply(
            f"**{STUDIO_NAME}** — подписка на платформу, если вы хотите **сами** управлять проектами, "
    ...
    def _pricing_reply(self) -> dict[str, Any]:
        ...
        lines.extend(
            [
                "",
                f"**{STUDIO_NAME}** (подписка) — от 49 €/мес, если хотите работать в платформе сами.",
                "",
