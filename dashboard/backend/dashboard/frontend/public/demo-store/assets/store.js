...
(function () {
    ...
    root.innerHTML = items.map(function (it) {
      ...
      return (
        '<article class="cart-line" data-id="' + it.id + '">' +
          ...
          '<button type="button" class="btn btn-ghost btn-sm" data-remove style="margin-top:0.5rem">Remove</button></div>' +
        "</article>"
      );
    }).join("");
  ...
  document.addEventListener("click", function (e) {
    ...
    if (t) {
      ...
      if (action === "checkout") {
        ...
        toast("Demo checkout — payments connect later");
      }
    }
  ...
  if (promo) {
    promo.addEventListener("click", function () {
      toast("Promo codes activate when checkout is live");
    });
  }
